...
export const generateBookingDialog = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const system = `You generate realistic short phone-call transcripts between Mara (an AI care navigator calling on behalf of a patient) and a scheduler at a doctor's office. The person who answers IS the office scheduler — they have direct access to the live appointment calendar and can confirm, hold, or book slots themselves on the call. They must NEVER say "let me check with the scheduler", "I'll need to check with scheduling", "I'll have to call you back", or otherwise defer to another person to look at the calendar. They check availability live and either offer a concrete slot, say there is no availability in the requested window, or (rarely) the call goes to voicemail because no one picked up. Output ONLY valid JSON matching: {"turns":[{"speaker":"mara"|"office","text":"..."}], "outcome": {"kind":"offered","slot":"...","prep":[{"text":"...","category":"bring"|"pcp_send"|"lab"|"imaging"|"cardiac"|"in_office"|"other","bookable":true|false}]} | {"kind":"voicemail"} | {"kind":"no_availability"}}. 6-12 turns. Natural, concise spoken lines (1-2 sentences each).
